...
// Component tìm kiếm phim (tương tự như cách bạn chọn phim)
...
export default function BannerForm() {
  ...
  const handleSave = async (values) => {
    // Validate file ảnh bắt buộc khi tạo mới
    if (!isEdit && !file) {
    ...
    try {
      ...
      if (file) formData.append("File", file, file.name); // Tên field khớp với API của bạn
  ...
  return (
    <Card loading={loading || bannerMutation.isLoading} title={
      <Space>
      ...
      <Form form={form} layout="vertical" onFinish={handleSave} onValuesChange={onValuesChange}>
        <Row gutter={24}>
          <Col xs={24} lg={16}>
            <Form.Item label="Movie ID" name="movie_id" rules={[{ required: true }]}>
              <MovieIdSelect />
            </Form.Item>
          ...
          <Col xs={24} lg={8}>
            <Form.Item label="Hình ảnh Banner">
              ...
              <Image src={img} fallback="https://via.placeholder.com/300x150?text=No+Banner" style={{ marginTop: 10, borderRadius: 8 }} />
            </Form.Item>
          </Col>
        </Row>
