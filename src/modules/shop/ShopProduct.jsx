...
import { fetchPromotionListAPI, fetchLocationListAPI, fetchBranchesAPI } from "services/general";
...
export default function ShopProduct(props) {  
  ...
  const filteredBranches = filters.area
    ? allBranches.filter((branch) => {
        ...
        return selectedArea.cumRap?.some((area) => branch.address?.includes(area));
      })
    : allBranches;
  ...
  return (
    <div className="container-fluid my-5 movie-list-container">
      {/* Filter Section */}
      <div className="container-fluid mb-4 p-4" style={{ backgroundColor: "#f5f5f5", borderRadius: "8px" }}>
        <Row gutter={[16, 16]}>
          {/* Khu vực */}
          <Col xs={24} sm={12} lg={4}>
            <div>
              <label style={{ display: "block", marginBottom: "8px", fontWeight: "500" }}>Khu vực</label>
              <Select
                placeholder="Chọn khu vực"
                style={{ width: "100%" }}
                allowClear
                value={filters.area}
                onChange={(value) => setFilters({ ...filters, area: value, theater: undefined })}
          ...
          <Col xs={24} sm={12} lg={4}>
            <div>
              <label style={{ display: "block", marginBottom: "8px", fontWeight: "500" }}>Rạp</label>
              <Select
          ...
          <Col xs={24} sm={12} lg={4}>
            <div>
              <label style={{ display: "block", marginBottom: "8px", fontWeight: "500" }}>Sắp xếp giá</label>
              <Select
          ...
          <Col xs={24} sm={12} lg={4}>
            <div>
              <label style={{ display: "block", marginBottom: "8px", fontWeight: "500" }}>Loại sản phẩm</label>
              <Select
                placeholder="Tất cả"
                style={{ width: "100%" }}
