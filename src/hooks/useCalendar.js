...
const daysShortArr = ['CN', 'Hai', 'Ba', 'Tư', 'Năm', 'Sáu', 'Bảy']
...
const useCalendar = (daysShort = daysShortArr, monthNames = monthNamesArr) => {
  ...
  //// lọc lấy gia trị date trong ogject gốc
  ...
  if (render_array && backup.length === 0) {
    ...
    if (render_array_today.length === 0) {
      ...
      if ((today.getMonth()+1) === 12) {
        setSelectedDate(
          (prevValue) =>
            new Date(prevValue.getFullYear() + 1, prevValue.getMonth() - 10, 1),
        )
      } else {
        setSelectedDate(
          (prevValue) =>
  ...
  if (backup.length !== 0) {
    if (todayFormatted[0] === '1' && todayFormatted[1] === '-') {
      ...
      const date_render_array_today = Object.values(render_array_today).map(
        (ele) => {
          return ele.map((col) => {
            return col.date
          })
